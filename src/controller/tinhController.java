...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		...
		String button = request.getParameter("but1");
		if(txta==null) {
			RequestDispatcher rd = request.getRequestDispatcher("truyenDuLieuTuForm.jsp");
			rd.forward(request, response);
		}else {
			HttpSession session = request.getSession();
			tinhBo tinh = new tinhBo() ;
			session.setAttribute("txta", txta);
			session.setAttribute("txtb", txtb);
			session.setAttribute("kq", tinh.tinh(button, txta, txtb));
			RequestDispatcher rd = request.getRequestDispatcher("truyenDuLieuTuForm.jsp");
			rd.forward(request, response);
